...
void Viewer::openPovFile() {
    ...
    *ini << "; Animation INI file generated by Bullet Physics Playground" << Qt::endl;
    *ini << QString("; %1 by %2").arg(timestamp, name) << Qt::endl << Qt::endl;
    *ini << "Input_File_Name=" << sceneName << ".pov" << Qt::endl;
    *ini << "Output_File_Name=" << sceneName << "-" << Qt::endl;
    *ini << "Output_to_File=On" << Qt::endl;
    *ini << "Pause_When_Done=Off" << Qt::endl;
    *ini << "Verbose=Off" << Qt::endl;
    *ini << "Display=On" << Qt::endl;
    *ini << "Width=720" << Qt::endl;
    *ini << "Height=480" << Qt::endl;
    *ini << "+FN" << Qt::endl;
    *ini << "+a +j0" << Qt::endl;

    // *ini << "Antialias_Threshold=0.3"

    *ini << "+L/usr/share/bpp/includes +L../../includes" << Qt::endl << Qt::endl;
    *ini << "Initial_Clock=" << _firstFrame << Qt::endl;
    *ini << "Final_Clock=" << _frameNum << Qt::endl;
    *ini << "Final_Frame=" << _frameNum << Qt::endl;

    *ini << "[240p]"  << Qt::endl <<  "Width=426" << Qt::endl << "Height=240"  << Qt::endl << "Antialias=Off" << Qt::endl;
    *ini << "[720p]" << Qt::endl << "Width=1280" << Qt::endl << "Height=720"  << Qt::endl
         << "Antialias=On" << Qt::endl << "Display=Off" << Qt::endl;
    *ini << "[1080p]" << Qt::endl << "Width=1920" << Qt::endl <<  "Height=1080" << Qt::endl
         << "Antialias=On" << Qt::endl << "Display=Off" << Qt::endl;
    *ini << "[2160p]" << Qt::endl << "Width=3840" << Qt::endl <<  "Height=2160" << Qt::endl
         << "Antialias=On" << Qt::endl << "Display=Off" << Qt::endl;
    ...
    *smain << "// Main POV file generated by Bullet Physics Playground" << Qt::endl;
    *smain << QString("// %1 by %2").arg(timestamp, name) << Qt::endl << Qt::endl;

    *smain << "#version 3.7;" << Qt::endl << Qt::endl;

    if (_pov_settings_inc != NULL && !_pov_settings_inc.isEmpty()) {
        *smain << "#include \"" + _pov_settings_inc + "\"" << Qt::endl << Qt::endl;
    }

    *smain << "#include concat(concat(\"" << sceneName << "-\",str(clock,-5,0)),\".inc\")" << Qt::endl << Qt::endl;
    ...
    *_stream << "// Include file generated by Bullet Physics Playground" << Qt::endl << Qt::endl;

    if (!mPreSDL.isEmpty()) {
        *_stream << mPreSDL << Qt::endl << Qt::endl;
    }


    if (_cam != NULL) {

        *_stream << "#declare use_focal_blur = " << _cam->getUseFocalBlur() << "; // 0=off 1=low quality 10=high quality" << Qt::endl << Qt::endl;

        if (_cam->getPreSDL() == NULL) {
            Vec pos = camera()->position();

            *_stream << "camera { " << Qt::endl
                     << "  location < " << pos.x << ", " << pos.y << ", " << pos.z << " >" << Qt::endl
                     << "  right - image_width/image_height*x" << Qt::endl;
...
*/
            ...
            *_stream << "  angle " << 180.0 * camera()->horizontalFieldOfView() / M_PI << Qt::endl;
            ...
            *_stream << "#if(use_focal_blur)" << Qt::endl
                     << "  aperture " << _cam->getFocalAperture() << Qt::endl
                     << "  blur_samples 10*use_focal_blur" << Qt::endl
                     << "  focal_point <"
                     << _cam->getFocalPoint().x() << ", "
                     << _cam->getFocalPoint().y() << ", "
                     << _cam->getFocalPoint().z() << "> "
                     << "  confidence 0.9+(use_focal_blur*0.0085)" << Qt::endl
                     << "  variance 1/(2000*use_focal_blur)" << Qt::endl
                     << "#end" << Qt::endl;

            *_stream << "}" << Qt::endl << Qt::endl;
        } else {
            *_stream << _cam->getPreSDL() << Qt::endl;
        }
    }
}
...
void Viewer::closePovFile() {
    if (!mPostSDL.isEmpty()) {
        *_stream << Qt::endl << mPostSDL << Qt::endl << Qt::endl;
    }
...
QString Viewer::toPOV() const {
    ...
    *s << "#include \"settings.inc\"" << Qt::endl << Qt::endl;

    if (!mPreSDL.isEmpty()) {
        *s << mPreSDL << Qt::endl << Qt::endl;
    }

    if (_cam != NULL) {

        *s << "#declare use_focal_blur = " << _cam->getUseFocalBlur() << "; // 0=off 1=low quality 10=high quality" << Qt::endl << Qt::endl;
        ...
        if (_cam->getPreSDL() == NULL) {
            ...
            *s << "  angle " << 180.0 * camera()->horizontalFieldOfView() / M_PI << Qt::endl;
    ...
    if (!mPostSDL.isEmpty()) {
        *s << Qt::endl << mPostSDL << Qt::endl << Qt::endl;
    }
