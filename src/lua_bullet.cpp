/**
 * @file lua_bullet.cpp
 * @brief Registers the Bullet Physics API with Lua through luabind.
 *
 * Besides the bindings themselves, this file carries the glue Bullet's own
 * types do not provide: wrappers that let a Lua table act as a Bullet
 * callback, stream and comparison operators luabind's policies insist on, and
 * overrides that stop Lua's garbage collector from deleting objects the
 * simulation still owns.
 */
