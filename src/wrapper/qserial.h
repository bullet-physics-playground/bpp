...
class QSerialPort : public QextSerialPort {
  Q_OBJECT
public:
  QSerialPort(QObject *parent = 0, int mode = EventDriven)
      : QextSerialPort(parent, QueryMode(mode)) {
    initial();
  }
  QSerialPort(const QString &name, QObject *parent = 0, int mode = EventDriven)
      : QextSerialPort(name, parent, QueryMode(mode)) {
    initial();
  }
  QSerialPort(const QString &name, const PortSettings &s, QObject *parent = 0,
              int mode = EventDriven)
      : QextSerialPort(name, s, parent, QueryMode(mode)) {
    initial();
  }
