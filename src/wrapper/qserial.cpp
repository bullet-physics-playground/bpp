...
SIGNAL_PROPERYT(lqserialport, disconnected, QSerialPort,
                "(const QextPortInfo&)")
...
void QSerialPort::initial() {
  ...
  connect(enumerator, SIGNAL(deviceDiscovered(QextPortInfo)), this,
          SIGNAL(connected(QextPortInfo)));
  connect(enumerator, SIGNAL(deviceRemoved(QextPortInfo)), this,
          SIGNAL(disconnected(QextPortInfo)));
}

void QSerialPort::deinitial() {
  disconnect(enumerator, SIGNAL(deviceDiscovered(QextPortInfo)), this,
             SIGNAL(connected(QextPortInfo)));
  disconnect(enumerator, SIGNAL(deviceRemoved(QextPortInfo)), this,
             SIGNAL(disconnected(QextPortInfo)));
}

LQextPortInfo lqextportinfo() {
  return class_<QextPortInfo>("QSerialInfo")
      .def(constructor<>())
...
LQPortSetting lqportsetting() {
  return class_<PortSettings>("QPortSetting")
      .def(constructor<>())
...
template <>
void table_init_general<QSerialPort>(const luabind::argument &arg,
                                     const object &obj) {
  lqserialport_init(construct<QSerialPort>(arg), obj);
}

QStringList validBaud() { return validValues<BaudRateType>(); }

LQextSerialPort lqextserialport() {
  return myclass_<QSerialPort>("QSerialPort", lqserialport_set_map)
      .def(constructor<>())
      ...
      .def("setDtr", tag_function<void(QSerialPort *)>(
                         boost::bind(&QSerialPort::setDtr, _1, true)))
      .def("setRts", &QSerialPort::setRts)
      .def("setRts", tag_function<void(QSerialPort *)>(
                         boost::bind(&QSerialPort::setRts, _1, true)))
      .def("lineStatus", &QSerialPort::lineStatus)
      .def("open", &QSerialPort::open)
      .def("open", tag_function<bool(QSerialPort *)>(boost::bind(
                       &QSerialPort::open, _1, QSerialPort::ReadWrite)))
      ...
      .def("write",
           (qint64(QSerialPort::*)(const char *, qint64)) & QSerialPort::write)
      .def("write", (qint64(QSerialPort::*)(const char *)) & QSerialPort::write)
      .def("write",
           (qint64(QSerialPort::*)(const QByteArray &)) & QSerialPort::write)
      .def("write",
           (qint64(QSerialPort::*)(const QByteArray &)) & QSerialPort::write)
      ...
      .property("queryMode", &QSerialPort::queryMode,
                &QSerialPort::setQueryMode)
      ...
      .property("flowControl", &QSerialPort::flowControl,
      ...
      .class_<QSerialPort>::property("portSetting", &QSerialPort::portSetting,
                                     &QSerialPort::setPortSetting)
      ...
      .scope[def("enumPort", &QextSerialEnumerator::getPorts),
      ...
      .enum_("DataBitsType")[value("DATA_5", DATA_5), value("DATA_6", DATA_6),
                             value("DATA_7", DATA_7), value("DATA_8", DATA_8)]
      .enum_(
          "ParityType")[value("PAR_NONE", PAR_NONE), value("PAR_ODD", PAR_ODD),
                        value("PAR_EVEN", PAR_EVEN),
                        value("PAR_MARK", PAR_MARK), // WINDOWS ONLY
                        value("PAR_SPACE", PAR_SPACE)]
      .enum_("StopBitsType")[value("STOP_1", STOP_1),
                             value("STOP_1_5", STOP_1_5), // WINDOWS ONLY
                             value("STOP_2", STOP_2)]
      .enum_("FlowType")[value("FLOW_OFF", FLOW_OFF),
                         value("FLOW_HARDWARE", FLOW_HARDWARE),
                         value("FLOW_XONXOFF", FLOW_XONXOFF)]
      .enum_("LineStatus")[value("LS_CTS", LS_CTS), value("LS_DSR", LS_DSR),
                           value("LS_DCD", LS_DCD), value("LS_RI", LS_RI),
                           value("LS_RTS", LS_RTS), value("LS_DTR", LS_DTR),
                           value("LS_ST", LS_ST), value("LS_SR", LS_SR)];
}

void QSerialPort::luaBind(lua_State *L) {
  luabind::module(L)[lqextserialport(), lqextportinfo(), lqportsetting()];
}
