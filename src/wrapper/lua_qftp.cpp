...
int lqftp_connectToHost(QFtp *w, const QString &host) {
  return w->connectToHost(host);
}
int lqftp_get1(QFtp *w, const QString &file) { return w->get(file); }
int lqftp_get2(QFtp *w, const QString &file, QFile *f) {
  return w->get(file, f);
}
int lqftp_get3(QFtp *w, const QString &file, QFile *f, QFtp::TransferType t) {
  return w->get(file, f, t);
}
...
int lqftp_put1(QFtp *w, QFile *dev, const QString &file) {
  return w->put(dev, file);
}
int lqftp_put2(QFtp *w, QFile *dev, const QString &file, QFtp::TransferType t) {
  return w->put(dev, file, t);
}
int lqftp_put3(QFtp *w, const QByteArray &data, const QString &file) {
  return w->put(data, file);
}
int lqftp_put4(QFtp *w, const QByteArray &data, const QString &file,
               QFtp::TransferType t) {
  return w->put(data, file, t);
}
...
int lqftp_connectToHost1(QFtp *w, const QByteArray &host, quint16 port) {
  return w->connectToHost(QString::fromLatin1(host), port);
}
int lqftp_connectToHost2(QFtp *w, const QByteArray &host) {
  return w->connectToHost(QString::fromLatin1(host));
}

int lqftp_get4(QFtp *w, const QByteArray &file) {
  return w->get(QString::fromLatin1(file));
}
int lqftp_get5(QFtp *w, const QByteArray &file, QFile *f) {
  return w->get(QString::fromLatin1(file), f);
}
int lqftp_get6(QFtp *w, const QByteArray &file, QFile *f,
               QFtp::TransferType t) {
  return w->get(QString::fromLatin1(file), f, t);
}
int lqftp_list2(QFtp *w, const QByteArray &dir) {
  return w->list(QString::fromLatin1(dir));
}

int lqftp_login3(QFtp *w, const QByteArray &user) {
  return w->login(QString::fromLatin1(user));
}
int lqftp_login4(QFtp *w, const QByteArray &user, const QByteArray &pwd) {
  return w->login(QString::fromLatin1(user), QString::fromLatin1(pwd));
}

int lqftp_mkdir(QFtp *w, const QByteArray &dir) {
  return w->mkdir(QString::fromLatin1(dir));
}

int lqftp_put5(QFtp *w, QFile *dev, const QByteArray &file) {
  return w->put(dev, QString::fromLatin1(file));
}
int lqftp_put6(QFtp *w, QFile *dev, const QByteArray &file,
               QFtp::TransferType t) {
  return w->put(dev, QString::fromLatin1(file), t);
}
int lqftp_put7(QFtp *w, const QByteArray &data, const QByteArray &file) {
  return w->put(data, QString::fromLatin1(file));
}
int lqftp_put8(QFtp *w, const QByteArray &data, const QByteArray &file,
               QFtp::TransferType t) {
  return w->put(data, QString::fromLatin1(file), t);
}

int lqftp_rawcommand(QFtp *w, const QByteArray &cmd) {
  return w->rawCommand(QString::fromLatin1(cmd));
}

int lqftp_remove(QFtp *w, const QByteArray &file) {
  return w->remove(QString::fromLatin1(file));
}
int lqftp_rename(QFtp *w, const QByteArray &oldname,
                 const QByteArray &newname) {
  return w->rename(QString::fromLatin1(oldname), QString::fromLatin1(newname));
}
int lqftp_rmdir(QFtp *w, const QByteArray &dir) {
  return w->rmdir(QString::fromLatin1(dir));
}
int lqftp_setProxy(QFtp *w, const QByteArray &host, quint16 port) {
  return w->setProxy(QString::fromLatin1(host), port);
}

LQFtp lqftp() {
  return class_<QFtp, QObject>("QFtp")
...
void lqurlinfo_setName(QUrlInfo *w, const QByteArray &name) {
  return w->setName(QString::fromLatin1(name));
}
QByteArray lqurlinfo_owner(QUrlInfo *w) { return w->owner().toLatin1(); }
void lqurlinfo_setOwner(QUrlInfo *w, const QByteArray &owner) {
  return w->setOwner(QString::fromLatin1(owner));
}

LQUrlInfo lqurlinfo() {
  return class_<QUrlInfo>("QUrlInfo")
      .def(constructor<const QUrlInfo &>())
      .def(constructor<>())
      //.def(constructor<const QString&,int,const QString&,const
      // QString&,qint64,const QDateTime&,const
      // QDateTime&,bool,bool,bool,bool,bool,bool>()) .def(constructor<const
      // QUrl&,int,const QString&,const QString&,qint64,const QDateTime&,const
      ...
      .property("lastModified", &QUrlInfo::lastModified,
      ...
      .property("permissions", &QUrlInfo::permissions,
...
LQTextCodec lqtextcodec() {
  return class_<QTextCodec, QTextCodec_wrap>("QTextCodec")
      .def("canEncode",
           (bool(QTextCodec::*)(const QString &) const) & QTextCodec::canEncode)
      ...
      .scope[def("availableCodecs", lqtextcodec_availableCodecs),
             ...
             def("codecForName",
                 (QTextCodec * (*)(const char *)) & QTextCodec::codecForName)];
}
...
LQTextDecoder lqtextdecoder() {
  return class_<QTextDecoder>("QTextDecoder")
...
LQTextEncoder lqtextencoder() {
  return class_<QTextEncoder>("QTextEncoder")
