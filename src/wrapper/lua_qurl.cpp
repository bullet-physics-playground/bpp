    ...
    for (iterator i(obj), e; i != e; ++i) {
      if (type(*i) == LUA_TTABLE) {
        ...
        if (p1 && p1 && type(p1) == LUA_TTABLE && type(p2) == LUA_TTABLE) {
          res.append(QPair<QByteArray, QByteArray>(byteArrayFromObject(p1),
                                                   byteArrayFromObject(p2)));
        }
      }
    }
    return res;
...
template <typename T> T __getValue(const T &t) { return (t); }
...
QByteArray QUrl_toPercentEncoding2(const QString &input,
...
LQUrl lqurl() {
  return myclass_<QUrl>("QUrl", lqurl_set_map)
      .def(constructor<>())
      ...
      .def("setEncodedUrl",
           (void(QUrl::*)(const QByteArray &, QUrl::ParsingMode)) &
               QUrl::setEncodedUrl)
      .def("setEncodedUrl",
           (void(QUrl::*)(const QByteArray &)) & QUrl::setEncodedUrl)
      ...
      .def("setUrl",
           (void(QUrl::*)(const QString &, QUrl::ParsingMode)) & QUrl::setUrl)
      ...
      .class_<QUrl>::property("encodedFragment", &QUrl::encodedFragment,
                              &QUrl::setEncodedFragment)
      .property("encodedHost", &QUrl::encodedHost, &QUrl::setEncodedHost)
      .property("encodedPassword", &QUrl::encodedPassword,
      ...
      .property("encodedQueryItems", &QUrl::encodedQueryItems,
                &QUrl::setEncodedQueryItems)
      .property("encodedUserName", &QUrl::encodedUserName,
                &QUrl::setEncodedUserName)
      .property("errorString", &QUrl::errorString)
      .property("queryItems", &QUrl::queryItems, &QUrl::setQueryItems)

      .scope[def("fromAce", &QUrl::fromAce),
             def("fromEncoded",
                 (QUrl(*)(const QByteArray &)) & QUrl::fromEncoded),
             def("fromEncoded",
                 (QUrl(*)(const QByteArray &, QUrl::ParsingMode)) &
                     QUrl::fromEncoded),
...
LQMimeData lqmimedata() {
  return myclass_<QMimeData, QObject>("QMimeData")
      .def(constructor<>())
      ...
      .class_<QMimeData, QObject>::property("colorData", &QMimeData::colorData,
...
Qt::DropAction QDrag_exec2(QDrag *w, Qt::DropActions supportedActions) {
  return w->exec(supportedActions);
}
Qt::DropAction QDrag_exec3(QDrag *w, Qt::DropActions supportedActions,
                           Qt::DropAction defAct) {
  return w->exec(supportedActions, defAct);
}
...
// template<>
// void table_init_general<QDrag>(const luabind::argument & arg, const object&
// obj)
//{
...
LQDrag lqdrag() {
  return myclass_<QDrag, QObject>("QDrag", lqdrag_set_map)
      ...
      .class_<QDrag, QObject>::property("mimeData", &QDrag::mimeData,
                                        &QDrag::setMimeData)
...
int QRegExp_indexIn2(QRegExp *w, const QString &str, int offset) {
  return w->indexIn(str, offset);
}
int QRegExp_lastIndexIn1(QRegExp *w, const QString &str) {
  return w->lastIndexIn(str);
}
int QRegExp_lastIndexIn2(QRegExp *w, const QString &str, int offset) {
  return w->lastIndexIn(str, offset);
}
int QRegExp_pos1(QRegExp *w) { return w->pos(); }
int QRegExp_pos2(QRegExp *w, int n) { return w->pos(n); }

LQRegExp lqregexp() {
  return class_<QRegExp>("QRegExp")
      .def(constructor<>())
      ...
      .property("capturedTexts",
                (QStringList(QRegExp::*)() const) & QRegExp::capturedTexts)
      .property("caseSensitivity", &QRegExp::caseSensitivity,
                &QRegExp::setCaseSensitivity)
      .property("errorString",
                (QString(QRegExp::*)() const) & QRegExp::errorString)
      ...
      .property("patternSyntax", &QRegExp::patternSyntax,
                &QRegExp::setPatternSyntax)
      .scope[def("escape", &QRegExp::escape)];
}
