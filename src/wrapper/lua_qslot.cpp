...
void QLuaSlot::emit_gen_signal() { emit general_signal(); }

void QLuaSlot::emit_gen_signal(QString s) { emit general_signal(s); }

void QLuaSlot::emit_gen_signal(int v) { emit general_signal(v); }
...
inline void gen_slot(const object &m_obj, const QString &m_method, T1 t1,
                     T2 t2) {
  try {
