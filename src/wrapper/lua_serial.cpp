...
template <> QString settingString(const PortSettings &s) {
  return PortSettingString(s);
}
...
MAP_ELEMENT(XONXOFF)
END_TYPE_MAP()
QString FlowString(FlowType flow) { return getString(FlowTypeMap, flow); }
...
template <typename T1, int size> QStringList getValues(T1 (&map)[size]) {
  QStringList list;
  for (int i = 0; i < size; i++) {
...
void tranverseSettingElement(T1 (&map)[size], pfnTranverse_t pfn, void *context,
                             const QString &prefix) {
  for (int i = 0; i < size; i++) {
    pfn(QString(map[i].desc).toLower().replace('_', '.'),
        prefix + QString("|%0").arg(map[i].value), false, context);
  }
}
...
template <> QStringList validValues<BaudRateType>() {
...
template <> QStringList validValues<DataBitsType>() {
...
template <> QStringList validValues<ParityType>() {
...
template <> QStringList validValues<StopBitsType>() {
...
template <> QStringList validValues<FlowType>() {
...
template <typename T, typename T2> inline void assign(T &d, T2 s) { d = (T)s; }

template <typename T, typename T2> inline bool same(T d, T2 s) {
  return d == (T)s;
}
