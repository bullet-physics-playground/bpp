  ...
  if (type(o1) == LUA_TUSERDATA) {
    ...
    if (!fun) {
      throw std::runtime_error(
          QString("member: %1 not defined").arg(name).toStdString());
      return false;
    }
    if (type(fun) != LUA_TFUNCTION) {
      throw std::runtime_error(
          QString("member: %1 is not function").arg(name).toStdString());
      return false;
    }
  } else {
    throw std::runtime_error(
        QString("parameter %1 is not a valid object").arg(index).toStdString());
    return false;
  }
  return true;
}
...
bool sigslot_connect(QObject *sender, const char *signal, QObject *recv,
                     const char *member) {
  ...
  // qDebug()<<(int)type(object(from_stack(__pL,
  // 1)))<<(int)type(object(from_stack(__pL, 2)))
  //         <<(int)type(object(from_stack(__pL,
  //         3)))<<(int)type(object(from_stack(__pL, 4)));
  if (iSig != -1 && iSlot != -1) {
    res = QObject::connect(sender, s.toLocal8Bit().data(), recv,
                           m.toLocal8Bit().data());
  } else if (iSig == -1 && iSlot == -1) {
    if (!checkMember(1, getName(signal).toLocal8Bit().data()))
      return false;
    if (!checkMember(3, getName(member).toLocal8Bit().data()))
      return false;
    QLuaSlot *p_signal = get_signal(sender, signal);
    QLuaSlot *p_slot = get_slot(recv, 3, member);
    res =
        QObject::connect(p_signal, p_signal->signal(), p_slot, p_slot->slot());
  } else if (iSig == -1) {
    // no signal, has slot
    if (!checkMember(1, getName(signal).toLocal8Bit().data()))
      return false;
    QLuaSlot *p_signal = get_signal(sender, signal);
    res = QObject::connect(p_signal, p_signal->signal(), recv,
                           m.toLocal8Bit().data());
  } else {
    // has signal, not slot
    if (!checkMember(3, getName(member).toLocal8Bit().data()))
      return false;
    QLuaSlot *p_slot = get_slot(recv, 3, member);
    res = QObject::connect(sender, s.toLocal8Bit().data(), p_slot,
                           p_slot->slot());
  }
  return res;
}
...
bool sigfunc_connect(QObject *sender, const char *signal, object func) {
  ...
  int iSig = sender->metaObject()->indexOfSignal(sig);
  bool res = false;
  if (type(func) != LUA_TFUNCTION && type(func) != LUA_TTABLE)
    return false;
  if (iSig == -1) {
    if (!checkMember(1, getName(signal).toLocal8Bit().data()))
      return false;
...
void emit_signal(QObject *sender, object obj) {
  ...
  QByteArray sig = QMetaObject::normalizedSignature(signal);
  // qDebug()<<"emit_signal "<<object_cast<const char*>(obj)<<"
  // "<<sender->property(sig).toString();
...
void register_classes(lua_State *L) {
  ...
  luabind::module(L)
      [lqobject(), lqwidget(),
       ...
       lqevent(), lqinputevent(), lqcloseevent(), lqcontextmenuevent(),
       lqdropevent(), lqdragmoveevent(), lqdragenterevent(), lqdragleaveevent(),
       lqkeyevent(), lqmouseevent(), lqpaintevent(), lqtimerevent(),
       lqwheelevent(), lqresizeevent(), lqshowevent(), lqhideevent(),

       lqmainwindow(), lqdockwidget(), lqstatusbar(),
       // lqtesttype(),

       lqdialog(), lqframe(), lqgroupbox(), lqsplitter(),

       lqabstractscrollarea(), lqmdisubwindow(), lqmdiarea(),
       lqsystemtrayicon(),

       lqicon(), lqaction(), lqmenu(), lqmenubar(), lqtoolbar(), lqcursor(),

       lqpoint(), lqline(), lqpen(),
       // lqpolygon(),
       lqrect(), lqsize(), lqcolor(), lqbrush(), lqfont(), lqmargins(),

       lqlayout(), lqstatckedlayout(), lqformlayout(), lqgridlayout(),
       lqboxlayout(), lqvboxlayout(), lqhboxlayout(), lqspaceritem(),

       lqlabel(), lqtextedit(), lqplaintextedit(), lqlineedit(),

       lqabstractbutton(), lqcheckbox(), lqpushbutton(), lqradionbutton(),
       lqtoolbutton(), lqbuttongroup(), lqkeysequence(),

       lqcombobox(), lqlistwidgetitem(), lqlistwidget(), lqtreewidgetitem(),
       lqtreewidget(), lqtablewidgetitem(), lqtablewidget(),
       lqtablewidgetselectionrange(),

       lqtabwidget(), lqtoolbox(),

       lqabstractspinbox(), lqspinbox(), lqdoublespinbox(),

       lqdate(), lqtime(), lqdatetime(), lqdatetimeedit(), lqdateedit(),
       lqtimeedit(), lqtimer(),

       lqpainter(), lqimage(), lqpixmap(), lqbitmap(),
       // lqvariant(),

       lqprocessenvironment(), lqprocess(), lqapplication(), lqdesktopwidget(),
       lqclipboard(), lqsound(), lqfilesystemwatcher(),

       lqabstractslider(), lqslider(), lqscrollbar(), lqdial(),

       lqprogressbar(), lqprogressdialog(),

       lqhostaddress(), lqnetworkproxy(), lqtcpserver(), lqabstractsocket(),
       lqtcpsocket(), lqudpsocket(), lqhostinfo(), lqnetworkinterface(),
       ...
       lqftp(), lqurlinfo(), lqtextcodec(), lqtextdecoder(), lqtextencoder(),

       lqfile(), lqtemporaryfile(), lqdir(), lqfileinfo(),
...
#ifdef HAS_LUA_GL
       lqglwidget(), lqglviewer(), lqglvec(), lquaternion(), lqcamera(),
#endif

       lqutil(),

       class_<QVariant_wrapper>("QVariant_wrapper").def(constructor<>()),
       def("connect",
           (bool (*)(QObject *, const char *, QObject *, const char *)) &
               sigslot_connect),
       def("connect",
           (bool (*)(QObject *, const char *, object)) & sigfunc_connect),
       def("emit_signal", (void (*)(QObject *, object)) & emit_signal),
       def("emit_signal", (void (*)(QObject *, object, object)) & emit_signal),
       def("emit_signal",
           (void (*)(QObject *, object, object, object)) & emit_signal),
       def("emit_signal",
           (void (*)(QObject *, object, object, object, object)) & emit_signal),

       def("toString", toString), def("toBase64", toBase64),
       def("fromBase64", fromBase64), def("fromString", fromString),

       def("bit_and", my_and), def("bit_or", my_or), def("bit_xor", my_xor)];
}
