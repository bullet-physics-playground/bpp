module("whistle", package.seeall);

function new(rad, hoehe, laenge, holder, text, textsize, mass)
  return OpenSCAD([===[

// Radius of the whistle in mm
rad    = ]===]..tostring(rad)..[===[; // 30 [20:50]

// Height of the whistle in mm
hoehe  = ]===]..tostring(hoehe)..[===[;  // 20 [15:30]

// Length of blow element in mm
laenge = ]===]..tostring(laenge)..[===[;  // 35 [20:50]

// Make a handle for whistle?
make_holder =  ]===]..tostring(holder)..[===[;  // 1 [0:1]

//text to put on bracelet
textshow = "]===]..text..[===["; // "Love"

// Textsize on whistle
textsize =  ]===]..tostring(textsize)..[===[;  // 12 [8:14]
...
// V1.2 st�rkerer Halter
// V2.0 Optimierunge f�r Parameter
// V2.1 Textparameter, Anpassung Innenkugel hohl
// V2.2 Innenkugel mit 1mm St�nder 
// V2.3 dynamischer Halter (scale)
// V2.4 innenreifen f�r rad>40 zum deckel machen, innenkugel true/false
...
module pfeife(name,sizename)
{
  ...
  if (true) // innere sphere - innenkugel mit st�ender
  {
