...
*   `18-dna-replication.lua`: Builds a DNA double helix (colored, base-paired nucleotide spheres on a twisted backbone) and simulates semi-conservative replication: a kinematic "helicase" marker travels up the helix breaking the hydrogen-bond rungs, while both strands are copied in real time as genuine `btPoint2PointConstraint` physics chains -- the leading strand continuously, the lagging strand as primer-started Okazaki fragments that "ligase" welds together once each fragment completes.
*   `19-dna-transcription.lua`: Builds the same kind of DNA double helix and simulates transcription: a kinematic "RNA polymerase" marker travels up the template strand, and for every base pair it passes, a real dynamic mRNA nucleotide (thymine replaced by uracil) is spawned and linked into a growing, gravity-sagging physics chain.
*   `19-dna-replication.lua`: Builds the same kind of DNA double helix and simulates semi-conservative replication: a kinematic "helicase" marker travels up the helix breaking the hydrogen-bond rungs, while both strands are copied in real time as genuine `btPoint2PointConstraint` physics chains -- the leading strand continuously, the lagging strand as primer-started Okazaki fragments that "ligase" welds together once each fragment completes.
