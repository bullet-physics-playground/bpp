-- =======================================================================
-- CONTROL PANEL -- live GUI foliotWeight slider via bpp's own
-- v:addParam / v:getParam, not a plain Lua variable.
-- =======================================================================
local PARAM_INFO = {
  -- Historically-accurate regulation lever for a verge/foliot: unlike a
  -- pendulum, the foliot has no gravity-dependent natural period, so
  -- real verge clocks were tuned by sliding weights along the foliot
  -- bar (changing its moment of inertia) rather than anything
  -- gravity-related. Applied symmetrically to both sides to keep the
  -- foliot balanced -- this is each weight's mass, not a position.
  foliotWeight = { min = 0.1, max = 20.0, step = 0.2,
                   comment = "mass added to EACH of the two foliot weights (kept equal/symmetric) -- more mass = more rotational inertia = slower beat, the traditional way a verge clock was regulated." },
}
...
-- Verge and Foliot -- classic verge escapement, swapped in for N7's
-- pendulum. Unlike the pendulum (swings about the X axis), the foliot
-- rotates about the VERTICAL axis (Y here), like a real verge clock's
-- balance. Geometry ported from Clock3.lua's calibrated Fork/escape
-- pair (Fork_com.stl == that script's Fork.com.stl): world pivot =
-- escape wheel's own world pos + (0,36,26), the fixed offset between
-- the two in that calibrated scene, since g2 below is NOT moving.
-- CAUTION: the pendulum's old sweep numbers above (now removed) don't
-- apply here -- a foliot has no gravity restoring force and typically
-- swings through a much larger arc than a small-angle pendulum, about
-- a different axis, so the downstream clearances (drum/gearA/hands,
-- staged assuming the old z=403.4 max) are NOT re-verified for this
-- geometry yet. Needs an actual run to check for interference,
-- particularly with gearA at z=459 -- uncomfortably close if the
-- foliot's ~129-unit arm radius swings through a wide angle.
-- Mirrored (about local x=0) rather than the original Fork_com.stl --
-- see g2's comment below for why, and DRIVE_SIGN further down.
--
-- FOLIOT_Y_ADJUST: small +/- nudge to the foliot's height (world Y),
-- for fine-tuning pallet/wheel engagement depth by hand -- edit this
-- number directly, no GUI slider. Applied to BOTH g1.pos and the
-- hinge's pivot0 together (not just g1.pos alone), since a hinge
-- constraint enforces pivot0 (in c's frame) and pivot1 (in g1's frame)
-- coincide in world space regardless of where g1.pos was set -- moving
-- one without the other would leave the constraint fighting the
-- placement instead of cleanly relocating the whole assembly.
local FOLIOT_Y_ADJUST = 2

g1 = Mesh("demo/mesh/WMS_E3_Pend.stl", 10, false)
g1.col = "#fa0"
g1.pos = btVector3(0,199 + FOLIOT_Y_ADJUST,330)
g1.friction = 0.1
g1.restitution = 0.0
-- Ported from Clock3.lua's calibrated setup -- constrains g1's own
-- linear motion (free-ish in x/y, locked in z) on top of the hinge
-- below. Unclear whether this is load-bearing stabilization or a
-- leftover hack from that older prototype; flagging rather than
-- silently dropping it, since the calibrated baseline used it.
g1.body:setLinearFactor(btVector3(0.1,0.1,0));
v:add(g1)
pivot0 = btVector3(0,49 + FOLIOT_Y_ADJUST,330)
axis0 = btVector3(0,1,0)
pivot1 = btVector3(0,0,0)
axis1 = btVector3(0,1,0)
...
-- ---------------------------------------------------------------------
-- Foliot regulation weights -- small spheres welded to the foliot's arm
-- tips, symmetric about the pivot (x=0) to keep it balanced. Placed at
-- local (+-115, 27, 0) on the foliot mesh itself: the arm tips
-- (|local x|>90) were found to sit in local y=14..42 across the full z
-- range, so y=27 (roughly the bar's mid-height) with x=115 (inside the
-- ~126 tip, leaving a little margin) puts these ON the bar, not
-- floating off it. Mass is live-adjustable via the "foliotWeight"
-- slider above -- Object.mass triggers Bullet's setMassProps
-- internally (confirmed in src/objects/object.cpp), so this updates
-- cleanly mid-sim without needing to recreate the weld.
-- ---------------------------------------------------------------------
local FOLIOT_WEIGHT_R = 8
local FOLIOT_WEIGHT_X = 115
local FOLIOT_WEIGHT_Y = 27

foliotWeightL = Sphere(FOLIOT_WEIGHT_R, v:getParam("foliotWeight"))
foliotWeightL.col = "#8888ff"
foliotWeightL.pos = btVector3(g1.pos.x - FOLIOT_WEIGHT_X, g1.pos.y + FOLIOT_WEIGHT_Y, g1.pos.z)
v:add(foliotWeightL)
local foliotWeightL_frameInG1 = btTransform(btQuaternion(0,0,0,1), btVector3(-FOLIOT_WEIGHT_X, FOLIOT_WEIGHT_Y, 0))
local foliotWeightL_frameInWeight = btTransform(btQuaternion(0,0,0,1), btVector3(0,0,0))
local foliotWeightL_weld = btGeneric6DofConstraint(g1.body, foliotWeightL.body, foliotWeightL_frameInG1, foliotWeightL_frameInWeight, true)
foliotWeightL_weld:setLinearLowerLimit(btVector3(0,0,0))
foliotWeightL_weld:setLinearUpperLimit(btVector3(0,0,0))
foliotWeightL_weld:setLimit(3, 0, 0)
foliotWeightL_weld:setLimit(4, 0, 0)
foliotWeightL_weld:setLimit(5, 0, 0)
v:addConstraint(foliotWeightL_weld)

foliotWeightR = Sphere(FOLIOT_WEIGHT_R, v:getParam("foliotWeight"))
foliotWeightR.col = "#8888ff"
foliotWeightR.pos = btVector3(g1.pos.x + FOLIOT_WEIGHT_X, g1.pos.y + FOLIOT_WEIGHT_Y, g1.pos.z)
v:add(foliotWeightR)
local foliotWeightR_frameInG1 = btTransform(btQuaternion(0,0,0,1), btVector3(FOLIOT_WEIGHT_X, FOLIOT_WEIGHT_Y, 0))
local foliotWeightR_frameInWeight = btTransform(btQuaternion(0,0,0,1), btVector3(0,0,0))
local foliotWeightR_weld = btGeneric6DofConstraint(g1.body, foliotWeightR.body, foliotWeightR_frameInG1, foliotWeightR_frameInWeight, true)
foliotWeightR_weld:setLinearLowerLimit(btVector3(0,0,0))
foliotWeightR_weld:setLinearUpperLimit(btVector3(0,0,0))
foliotWeightR_weld:setLimit(3, 0, 0)
foliotWeightR_weld:setLimit(4, 0, 0)
foliotWeightR_weld:setLimit(5, 0, 0)
v:addConstraint(foliotWeightR_weld)

-- Escapement Wheel -- hinged on Z, driven by the drum/weight below.
-- Mirrored (about local x=0), same as g1 above and for the same reason:
-- DRIVE_SIGN=1 is required elsewhere in this file to make the clock
-- hands rotate the correct way (second hand is welded straight onto
-- g2), but the UNMIRRORED escape_com.stl/Fork_com.stl pair only
-- engaged correctly under DRIVE_SIGN=-1. Mirroring BOTH meshes about
-- the same plane (x=0 -- both bodies sit at world x=0, so this doesn't
-- shift either one) reverses the pallet/tooth engagement chirality as
-- a matched pair, restoring correct engagement under DRIVE_SIGN=1
-- instead of needing to flip the sign (which would reverse the hands).
g2 = Mesh("demo/mesh/WMS_E3_Wheel.stl", 100, false)
g2.col = "#b00"
...
local DRUM_R  = 40
local DRUM_TH = 15
local WEIGHT_M = 0.06 -- WMS
local WEIGHT_R, WEIGHT_H = 22, 80
local DRIVE_SIGN = 1 -- back to N7's original sign (correct hand rotation) -- now valid because g1/g2 use mirrored meshes above, not -1.
...
-- OS CLOCK SYNC: gearD's validated mesh-family with gearC repeats
-- every 3.75deg (its own tooth pitch) -- 3.58 (found by brute-force
-- polygon-overlap search, replicating involute_gear_sdl's exact
-- geometry including its clockwise rotate_point() convention,
-- confirmed near-zero interpenetration -- 0.041, pure discretization
-- noise relative to gearD's actual size) is ONE member of that family;
-- every 3.58+k*3.75, for any integer k, is EQUALLY validly meshed. So:
-- compute the ideal (continuous, not rounded) raw angle for the
-- current wall-clock minute using this file's clock-face convention
-- (deg=0 -> 12 o'clock -> raw=90-deg), using the TOTAL minutes since
-- 12:00 (not wrapped to one hour, so the hour hand -- derived below --
-- gets the right hour too), then snap to the nearest family member.
-- Worst-case rounding error is half the family spacing (1.875deg,
-- ~19 real seconds on the minute hand).
--
-- This whole approach (and the two validated linear ratio formulas
-- used below for idlerGear/hourWheel) replaces an earlier version
-- that independently rounded gearD relative to gearC and derived
-- hourWheel via an unvalidated /12 ratio formula -- that version never
-- actually checked whether gearE/idlerGear/hourWheel's mesh was
-- geometrically valid, and it wasn't (confirmed: real tooth
-- interpenetration).
--
-- BASELINE+RATIO FRAMEWORK: GEARD_BASELINE (3.58) is DEFINED to be
-- "gearD's zero" -- not 12 o'clock, just wherever the gear itself
-- properly meshes with gearC. minuteHand is separately welded with a
-- FIXED offset so it reads exactly 12:00 AT that reference (see
-- MINUTE_HAND_OFFSET below) -- and because every downstream angle
-- (idler, hourWheel, both hands) is a fixed ratio away from gearD's
-- rotation FROM ITS OWN BASELINE, rotating gearD by theta degrees from
-- baseline rotates everything else by the correct, proportional
-- amount too, and the mesh never falls out of validity. theta =
-- -totalMinutes*6 (negative because increasing time needs decreasing
-- raw, this file's "raw=90-clockDeg" convention), then snapped to the
-- nearest multiple of gearD's own 3.75deg tooth pitch so the target
-- lands on an ACTUALLY validated meshing position.
...
-- mesh (unlike N2, which at least had a base 0.3) -- confirmed
-- directly: at zero backlash, gearE-idler's valid window was only
-- 0.22deg and idler-hourWheel's was a razor-thin 0.02deg, tighter than
-- N2's original problem was. Fixed the same way: 0.3 on idler_r1
-- (matches N2's working value; gearE-idler's window stays solid,
-- 1.44-1.56deg, across the whole range tested) and 1.5 on idler_r2
-- (checked BOTH window width -- widens to 0.50deg, 25x the original --
-- AND actual tooth engagement depth together, landing at 88% coverage
-- across a full tooth pitch, not just "wider is better" -- a naive
-- much-larger value was already confirmed elsewhere in this project to
-- leave almost no real engagement at all).
...
-- confirmed near-zero, not just the one calibration point.
...
-- ---------------------------------------------------------------------
-- SLOW-MOVING FOLIOT-WEIGHT CONTROLLER: nudges the "foliotWeight"
-- slider, once every BEATS_PER_BLOCK beats, so the REAL (wall-clock)
-- time for a whole block of beats drifts toward TARGET_BLOCK_SECONDS.
-- This is the historically-correct lever for a verge/foliot (unlike
-- the pendulum version, which used gravity) -- calls
-- setParam("foliotWeight", ...), and the mass-update block above
-- (search "curFoliotWeight") picks the new value up on the next frame
-- and reapplies it to both weights, exactly as if a person had dragged
-- the slider by hand.
--
-- Sign: MORE mass on the foliot means MORE rotational inertia, which
-- means a SLOWER beat (confirmed empirically -- see the mass=1.0->4.0
-- test in the conversation this was built from: beats visibly
-- lengthened right after). So, opposite of the old gravity controller,
-- a block running SLOWER than target (positive error) needs a
-- NEGATIVE weight nudge (less mass) to speed it back up.
--
...
-- Scaled to keep the same gain-to-slider-range ratio the gravity
-- controller used (300 out of a 200..4000 range, i.e. ~7.9% of span per
-- BEATS_PER_BLOCK), applied instead to foliotWeight's much smaller
-- 0.1..20.0 range. Untested/rough starting point -- watch a training run
-- and retune if convergence looks too slow or too twitchy.
local WEIGHT_RANGE              = PARAM_INFO.foliotWeight.max - PARAM_INFO.foliotWeight.min   -- 19.9
local BLOCK_GAIN                = (0.0789 * WEIGHT_RANGE) / BEATS_PER_BLOCK   -- weight units per second of block error, before clamping (~0.0374)
local MAX_WEIGHT_STEP_PER_UPDATE = PARAM_INFO.foliotWeight.step   -- reuse the slider's own step size as the hard per-update cap
local CONTROL_WARMUP_BEATS     = 5      -- beats since SIM START (not "since training started" anymore) to skip before block monitoring starts -- block monitoring now runs continuously regardless of training state, see the postSim block below.
...
-- controller above becomes active and starts adjusting the foliot
-- weight); press 'G' once it looks stable to lock the weight at its
-- current value and stop touching it, session-wide. Starts OFF by
-- default -- a headless run never presses a key, so the weight just
-- stays at its startup value.
-- ---------------------------------------------------------------------
...
v:addShortcut("T", function(N)
  if training then
    print("[tuning] already running -- press 'G' to lock in the current foliot weight, or let it keep tuning")
    return
  end
  training = true
...
v:addShortcut("G", function(N)
  ...
  training = false
  print(string.format("[tuning] LOCKED at frame %d -- foliotWeight=%.2f, held fixed from here on", N, v:getParam("foliotWeight")))
  print(string.format("[tuning] foliotWeight ranged from %.2f to %.2f (span %.2f) over this tuning session",
                       trainingWeightMin, trainingWeightMax, trainingWeightMax - trainingWeightMin))
end)
...
v:setHelpText(
  "T: Start foliot-weight tuning (adjusts weight toward a 60s/" .. BEATS_PER_BLOCK .. "-beat target)\n" ..
  "G: Lock the foliot weight at its current value and stop tuning\n" ..
  "S: Toggle beat sound (tick/tock) on/off"
)

local lastFoliotWeight = v:getParam("foliotWeight")

-- ---------------------------------------------------------------------
-- HAND SWEEP: gearD is kinematically eased from 3.58 (the k=0
-- reference, its construction-time pose above) to GEARD_RAW_TARGET
-- (the real current time) over SWEEP_SECONDS. gearE, minuteHand,
-- idlerGear, hourWheel, hourHand are all re-derived from gearD's
-- CURRENT (in-progress) angle every frame using the same validated
-- linear formulas used at construction time -- not independently
-- animated -- so they stay correctly, continuously meshed throughout
-- the whole sweep. Since this is pure kinematic/formula-driven motion
-- (not physics propagating contact forces through the train), there's
-- no physical speed limit -- a fixed, short duration works regardless
-- of how much rotation GEARD_RAW_TARGET represents.
-- ---------------------------------------------------------------------
...
v:postSim(function(N)
  if not sweepDone then
    ...
    if elapsed >= SWEEP_SECONDS then
      ...
      -- starts.
      lastAngVel = g1.body:getAngularVelocity().y
  ...
  local curFoliotWeight = v:getParam("foliotWeight")
  if curFoliotWeight ~= lastFoliotWeight then
    foliotWeightL.mass = curFoliotWeight
    foliotWeightR.mass = curFoliotWeight
    print(string.format("frame %d: foliot weight slider moved %.2f -> %.2f (each side)", N, lastFoliotWeight, curFoliotWeight))
    lastFoliotWeight = curFoliotWeight
  end
  ...
  -- .y, not .x: the foliot rotates about the vertical (Y) axis, unlike
  -- the pendulum it replaced (which swung about X).
  --
  -- Gated behind sweepDone: zero motor torque during the sweep wasn't
  -- enough on its own to prevent beats (residual settling motion from
  -- the rest of the gear train still nudges the foliot enough to cross
  -- zero angular velocity a couple times), so beat detection itself is
  -- simply skipped entirely until the sweep is done, rather than
  -- fighting to perfectly freeze g1's physics.
  if sweepDone then
    local av = g1.body:getAngularVelocity().y
    ...
    if beatDurationSim >= MIN_BEAT_DURATION_SIM then
      if beatCount > 0 then
        ...
        -- CONTROL_WARMUP_BEATS is now "beats since sim start" (not
        -- "since training started" -- it applies regardless of whether
        -- training is ever turned on).
        if beatCount >= CONTROL_WARMUP_BEATS then
          ...
          if blockBeatCount >= BEATS_PER_BLOCK then
            ...
            elseif not training then
              ...
              -- error for visibility without touching foliotWeight.
            ...
            -- trainingWeightMin/Max are nil until 'T' has been pressed
            -- at least once, so guard this line or it'll error before that.
            if training then
              print(string.format("  [beat control] foliotWeight range so far this session: %.2f to %.2f (span %.2f)",
                                   trainingWeightMin, trainingWeightMax, trainingWeightMax - trainingWeightMin))
            end
