...
-- not tick during the gear spin-up animation (see the postSim block
-- near the bottom). Full torque is applied once that animation
-- completes.
...
-- OS CLOCK SYNC, using the baseline+ratio framework: gearD's own
-- validated meshing reference (129.58, GEARD_BASELINE below) is
-- DEFINE to be "gearD's zero" -- not 12 o'clock, just wherever the
-- gear itself properly meshes with gearC. minuteHand is separately
-- welded with a FIXED offset so it reads exactly 12:00 AT that
-- reference (see MINUTE_HAND_OFFSET below) -- and because every
-- downstream angle (idler, hourWheel, both hands) is a fixed ratio
-- away from gearD's rotation FROM ITS OWN BASELINE, rotating gearD by
-- theta degrees from baseline rotates everything else by the correct,
-- proportional amount too, and the mesh NEVER falls out of validity
-- (the ratios preserve it automatically, for any theta, not just
-- theta=0) -- confirmed algebraically: this reduces exactly to the
-- standard hourClockDeg = (hour12+min/60)*30 formula, not
-- independently re-derived per case.
--
-- theta (gearD's rotation away from its own baseline) = -totalMinutes*6:
-- negative because increasing time needs decreasing raw (this file's
-- "raw=90-clockDeg" convention), same sign relationship gearD always
-- had, just measured from GEARD_BASELINE now instead of from 90.
-- gearD's target angle repeats every 3.75deg (its tooth pitch) from
-- baseline -- snap theta to the nearest multiple of 3.75 so the target
-- lands on an ACTUALLY validated meshing position, not an arbitrary
-- continuous one. Worst-case rounding error: half the family spacing
-- (1.875deg, ~19 real seconds on the minute hand).
GEARD_BASELINE = 129.58   -- global -- gearD's own validated meshing reference; "gearD's zero", not 12 o'clock
local t = os.date("*t")
...
-- Matches gearD's raw angle directly (the weld's frame is a pure Z
-- translation -- same rotation, WITH a fixed offset baked in so the
-- hand reads exactly 12:00 at gearD's own baseline (see
-- MINUTE_HAND_OFFSET below) -- not relying on the weld constraint to
-- pull it into place over the first few physics steps. Kinematic +
-- no-contact-response for the sweep -- confirmed by earlier direct
-- measurement that leaving it dynamic while gearD is kinematic lets it
-- sag under gravity (the weld isn't stiff enough to resist gravity
-- while the gear train sits unloaded).
...
-- IDLER_HOURWHEEL_BACKLASH -- this specific mesh has a MUCH finer tooth
-- pitch on the hourWheel side (96 teeth vs the 8-tooth gears everywhere
-- else in this train), which geometrically leaves very little angular
-- clearance for a given radial backlash -- confirmed directly: at
-- BACKLASH=0.3 the valid (non-overlapping) meshing window was only
-- 0.06deg wide, narrow enough to let real contact-solver noise
-- periodically knock it a whole tooth off, causing the hour hand to
-- visibly lag behind over time. 1.5 was chosen by checking BOTH the
-- safe-window width AND actual tooth engagement depth together (a
-- naive "just widen it a lot" first attempt at 5.0 left almost no
-- tooth-tip overlap at all -- confirmed: contact became sparse and
-- intermittent instead of continuous). At 1.5: window widens to
-- 0.48deg (8x the original) while engagement stays continuous across
-- 87% of a full tooth pitch (vs 97% at the original 0.3).
...
-- Tracks gearE via a validated linear formula -- idler_raw = 284.63 -
-- gearE_raw -- derived from the single brute-force-confirmed reference
-- point (gearE_raw=129.58 -> idler_raw=163.10, zero overlap) combined
-- with the theoretically-correct -1x ratio (equal 8-tooth external
-- gears), then re-validated by checking zero overlap holds at SIX
-- other, very different gearE_raw values (up to +/-360deg away) --
-- confirmed exact in every case, not just the one calibration point.
...
idlerGear.body:setMotionState(btDefaultMotionState(
  btTransform(btQuaternion(btVector3(0,0,1), math.rad(284.63 - GEARD_RAW)), idlerGear.pos)))
...
-- -idler_raw/12 + 79.7708 -- derived from the brute-force-confirmed
-- reference point (idler_raw=163.10 -> hourWheel_raw=129.48, zero
-- overlap) combined with the theoretically-correct -1/12 ratio
-- (8-tooth idler : 96-tooth hourWheel, external mesh, reversed
-- direction), re-validated at six other very different idler_raw
-- values -- exact zero overlap held in every case.
hourWheel.body:setCollisionFlags(2 + 4)
hourWheel.body:setActivationState(4)
HOUR_WHEEL_RAW = -(284.63 - GEARD_RAW)/12 + 79.7708   -- global -- current angle during the sweep, updated each frame; also read by hourHand below
...
-- ---------------------------------------------------------------------
-- Diagnostics: tooth-skip detection and beat measurement (sim vs real
-- OS time). (Gear-slip-per-stage and pallet/wheel penetration-depth
-- diagnostics removed -- superseded by the [hand check] line below,
-- which reads actual hand position directly instead of an indirect
-- per-mesh ratio, and was never reachable, respectively.)
-- ---------------------------------------------------------------------
-- Tooth-skip detection on the escapement wheel itself (g1/g2 contact),
-- and wheelTurn tracking for the falling-weight visual -- both still
-- active, unlike the removed gear-slip and penetration diagnostics above.
...
local osStartCapture = os.date("*t")  -- PROBE: temporary, transferred into spinUp below so it doesn't add a new postSim upvalue
local beatCount = 0
...
local BLOCK_ERROR_DEADBAND     = 0.5    -- block |error| (seconds) must exceed this before any correction happens
...
soundMuted = false        -- true after an 'S' press, until the next 'S' press toggles it back (global, not local -- postSim is already at Lua's 60-upvalue ceiling)
...
-- ---------------------------------------------------------------------
-- HAND SWEEP: gearD is kinematically eased from 129.58 (the k=0
...
-- the whole sweep, not just at the start and end. Since this is pure
-- kinematic/formula-driven motion (not physics propagating contact
-- forces through the train), there's no physical speed limit -- a
-- fixed, short duration works regardless of how much rotation
-- GEARD_RAW_TARGET represents.
-- ---------------------------------------------------------------------
local SWEEP_SECONDS = 5.0
local GEARD_RAW_START = GEARD_BASELINE
local spinUp = { sweepT0 = nil, sweepDone = false, torqueT0 = nil, torqueDone = false,
                  osStartSeconds = osStartCapture.hour*3600 + osStartCapture.min*60 + osStartCapture.sec }
...
local function applySweepAngles(gearDRawNow)
  ...
  local idlerRawNow = 284.63 - gearDRawNow
  ...
  local hourWheelRawNow = -idlerRawNow/12 + 79.7708
...
v:postSim(function(N)
  if not spinUp.sweepDone then
    if spinUp.sweepT0 == nil then
      spinUp.sweepT0 = v:getTime()
    end
    local elapsed = v:getTime() - spinUp.sweepT0
    if elapsed >= SWEEP_SECONDS then
      ...
      spinUp.sweepDone = true
      print(string.format("frame %d: hand sweep complete (%.1fs) -- whole minute/hour train now driven by normal physics, motor torque ramping up next", N, elapsed))
    else
  ...
  elseif not spinUp.torqueDone then
    if spinUp.torqueT0 == nil then
      spinUp.torqueT0 = v:getTime()
    end
    local elapsed = v:getTime() - spinUp.torqueT0
    if elapsed >= 1.0 then
      con0:enableAngularMotor(true, DRIVE_SIGN * 100, DRIVE_TORQUE)
      -- Fresh baseline for beat detection -- otherwise the first real
      -- beat's reported duration would span the whole ramp window
      -- instead of being measured from when the escapement is
      -- actually running at full torque.
      lastAngVel = g1.body:getAngularVelocity().z
      lastBeatTime = N * v.timeStep
      lastBeatClock = v:getTime()
      spinUp.torqueDone = true
      print(string.format("frame %d: motor torque ramp complete (%.1fs) -- now at full DRIVE_TORQUE=%.1f, beat detection starting", N, elapsed, DRIVE_TORQUE))
    else
      local frac = spinUpEase(elapsed / 1.0)
      con0:enableAngularMotor(true, DRIVE_SIGN * 100, DRIVE_TORQUE * frac)
    end
  end
  ...
  if curGravity ~= lastGravity then
    ...
    -- Only actually re-enable the motor at full torque if the ramp is
    -- done -- otherwise dragging gravity mid-animation/mid-ramp would
    -- jump straight to full torque, the exact abrupt-jump problem the
    -- ramp exists to avoid. DRIVE_TORQUE is still recomputed and ready
    -- for whenever the ramp (or a future gravity change) applies it.
    if spinUp.torqueDone then
      con0:enableAngularMotor(true, DRIVE_SIGN * 100, DRIVE_TORQUE)
    end
  ...
  -- Gated behind torqueRampDone (not just spinUpDone): zero motor
  -- torque during the hand animation wasn't enough on its own to
  -- prevent beats (residual settling motion from the rest of the gear
  -- train can still nudge g1 enough to cross zero angular velocity a
  -- couple times), and the motor torque ramp that follows the hand
  -- animation could plausibly do the same, so beat detection is
  -- skipped entirely until the escapement is running at full torque.
  if spinUp.torqueDone then
      ...
      if beatCount > 0 then
        ...
        if beatCount >= CONTROL_WARMUP_BEATS then
          if blockStartClock == nil then
            -- FIX (off-by-one): blockStartClock must be the timestamp of
            -- the beat BEFORE this one (lastBeatClock, not yet overwritten
            -- to nowClock at this point in the frame -- that happens later,
            -- see below), so that BEATS_PER_BLOCK beats later, blockElapsed
            -- spans exactly BEATS_PER_BLOCK real intervals, not
            -- BEATS_PER_BLOCK-1. The original code used nowClock here,
            -- which starts the clock ON this beat rather than one beat
            -- before it, undercounting every block by one full beat period.
          ...
          if blockBeatCount >= BEATS_PER_BLOCK then
            ...
            -- this codebase's convention (confirmed via the frame-1 sync
            -- check); angle decreases clockwise. Minute hand: one 6deg/min
            -- sweep, so its angle alone gives minutes-past-the-hour
            -- directly. Hour hand: one 30deg/hour sweep (12-hour dial), so
            -- its angle gives a continuous hour+fraction reading, which we
            -- split into an indicated hour and an indicated minute-within-
            -- that-hour for the same side-by-side comparison.
            ...
            local monotonicSeconds = (spinUp.osStartSeconds + (v:getTime() - simClockStart)) % 86400
