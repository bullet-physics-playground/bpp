...
-- This is a simulation of a walker driven by Bill Spears's own
-- Hoecken-slider proportions ("Hoeckens-Spears" in linkage.lua:
-- compute_hoeckens_spears, crank=36, ground=46.95, bar=168.98) --
-- CONVERTED from an earlier version of this file that used the
-- ORIGINAL 1867 Hoecken slider's classical "a, 2a, 10a" ratios
-- instead. Both are the exact same mechanism topology (a crank drives
-- a long rigid bar threaded through a fixed pivot, ground_angle=90 in
-- both -- see the SLIDER JOINT note below for how that's built from
-- Bullet constraints); only the relative proportions of crank : ground
-- : bar differ. Ported from Chebyshev's Plantigrade Machine (shown at
...
--
-- HOECKEN-SPEARS CONVERSION, precisely: linkage.lua's own two Hoecken-
-- slider entries use genuinely different crank:ground:bar RATIOS, not
-- just different scales of the same shape --
--   Original:        crank : ground : bar = 1     : 2.0     : 10.0
--   Hoeckens-Spears:  crank : ground : bar = 1     : 1.30417 : 4.69389
--     (46.95/36 and 168.98/36, from Spears's own crank=36 ground=46.95
--     bar=168.98 -- the same crank=36 his Chebyshev-Spears and Spears
--     4Bar-1 entries in linkage.lua also use, suggesting a real
--     hardware build's own characteristic scale, not a textbook
--     Hoecken rescale.)
-- Run head-to-head (linkage.lua's own computeMetrics, 720 samples/
-- cycle, both normalized to unit crank so they're comparable pound-for-
-- pound): Hoeckens-Spears gets a substantially higher duty cycle (84.0%
-- vs. the original's 69.3% of the cycle spent in ground-contact stance)
-- and near-identical monotonicity (0.9901 vs 1.0000, both essentially
-- backtrack-free) and flatness (4.84% vs 3.62%) -- at the cost of a
-- shorter stride per unit crank (normalized X span 6.061 vs 8.324, ~73%
-- of the original's reach). RENORMALIZED here to cancel that stride
-- cost out: every length below is Spears's own ratio scaled up by
-- k=1.373369 (solved numerically so the swept X-span exactly matches
-- the ORIGINAL mechanism's own stride at crank=1, both computed via the
-- walker's own buildLinkage formula, not just linkage.lua's abstract
-- one -- verified to match to 4 decimal places: 8.3237 vs 8.3236) --
-- so this version keeps the original's exact stride length while
-- picking up Hoeckens-Spears's higher duty cycle for free.
--
-- Ground link  g = 1.791101  -- O2 (crank pivot) -> O4 (slider pivot), DIRECTLY
--                          UPWARD from O2 (g_ang=90), matching
--                          compute_hoeckens_spears's own ground_angle=90
-- Crank        a = 1.373369  -- O2 -> A
-- Slider bar   L = 6.446439  -- A  -> B, threaded through O4 (crank:ground:bar = 1 : 1.30417 : 4.69389, Spears's own ratio, scaled by k=1.373369 for equal stride -- see conversion note above)
-- Pendant      p = 10.2425  -- B  -> C, hinged and hanging free (no motor) -- shortened from the original's 14.0 (see the AMPLITUDE / SCALE note below for why: B sits lower with this proportion, so a shorter pendant reproduces the same foot-to-floor clearance against the same fixed floor_top_y)
--
...
-- to the side.
--
-- AMPLITUDE / SCALE: Hoeckens-Spears's own ratio (crank : ground : bar
-- = 1 : 1.30417 : 4.69389), scaled by k=1.373369 so the swept stride
-- exactly matches the original mechanism's -- see the HOECKEN-SPEARS
-- CONVERSION note above for the full derivation. Checked numerically
-- (sweeping the crank through a full rotation, no physics run, using
-- this file's own buildLinkage formula): with the vertical orientation
-- above, B's range comes out to roughly Y in [5.43, 8.17] (a noticeably
-- LOWER band than the original's [9.19, 11.25] -- the shorter bar,
-- relative to this proportion's own crank, doesn't reach nearly as far
-- past the fixed pivot) and X in [-4.16, +4.16] relative to the mount
-- (essentially IDENTICAL to the original's X range, by construction --
-- that's the whole point of the stride-matching renormalization). That
-- X range comfortably clears the neighboring leg (linkage_spacing=10
-- apart, checked for both x_offset=0 and x_offset=10 -- no overlap).
-- Since B now sits lower, p_len was shortened to 10.2425 (from the
-- original's 14.0) to reproduce the SAME foot-to-floor clearance
-- against the SAME fixed floor_top_y=-5.7: B.ymin - p_len works out to
-- -4.815 either way (0.885 above the floor), so the foot should touch
-- down at essentially the same point in its cycle, and reach the same
-- depth, as the original version did -- only the duty cycle (how much
-- of the cycle it stays down there) should actually differ, per the
-- HOECKEN-SPEARS CONVERSION note's metrics comparison.
--
...
-- NOTE (post Hoecken-Spears conversion): these percentages were computed
-- for the ORIGINAL lengths (crank=1.0, bar/coupler=10.0, pendant=14.0),
-- not re-derived for the current ones (1.373369, 6.446439, 10.2425) --
-- the widths themselves (0.3, 1.2, 1.0) are unchanged below, only the
-- lengths shrank, which should make the I_xx/I_zz tradeoff even MORE
-- favorable (a shorter rod's I_zz drops faster with length^2 than a
-- fixed width's I_xx does), but that's a qualitative expectation, not a
-- checked number -- re-run this sweep if the exact percentages matter.
...
-- 0.4) -- comfortable margin, not a near-miss (also not re-verified
-- against the new, shorter lengths -- flagged for the same reason).
--
...
local g_len, a_len, f_len, p_len = 1.791101, 1.373369, 6.446439, 10.2425   -- Hoeckens-Spears ratio (1 : 1.30417 : 4.69389) scaled by k=1.373369 for equal stride to the original a=1,g=2,L=10 version; p_len shortened from 14.0 to reproduce the same foot-to-floor clearance against the same fixed floor_top_y -- see the HOECKEN-SPEARS CONVERSION header note for the full derivation
...
-- ---------------------------------------------------------------------
-- The bar's own tip B stays HIGH (y in roughly [5.43, 8.17]) over a
-- full crank rotation at this scale -- lower than the original ratio's
-- B range ([9.19, 11.25]) but still well clear of the cube -- checked
-- numerically (Lua sweep, no NaNs/degenerate geometry across a full
-- rotation for both x_offset=0 and x_offset=10), not from a live
-- physics run. See the HOECKEN-SPEARS CONVERSION note up top for the
-- full derivation of these lengths.
--
-- floor_top_y is DELIBERATELY NOT re-derived from p_len -- it's a fixed
-- literal, -5.7, unchanged from the original-ratio version of this
-- file. The HOECKEN-SPEARS CONVERSION note up top explains why that's
-- still correct here: p_len was deliberately shortened (14.0 -> 10.2425)
-- specifically so B.ymin - p_len reproduces the exact same -4.815
-- foot-low-point (0.885 above this same fixed floor) that the original
-- ratio achieved with its own, taller B range and longer pendant --
-- two different geometries tuned to meet at the same floor contact
-- point, rather than moving the floor to chase a new geometry.
