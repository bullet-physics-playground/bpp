...
-- also sidesteps the whole "bury the marker at this bump's own height"
-- problem the old marker code above wrestled with (see the commented-out
-- terrainHeight() line that used to be in the preSim hook below) -- a
-- colored triangle IS the terrain surface at that point, at exactly the
-- right height, by construction. There's also nothing to v:remove() any
-- more -- see clearTrail() below.
--
...
-- not drawing anything. (This file's floor is only 600x600 with no Z
-- lock on the cube, unlike some other files in this series, so both i
-- and j can legitimately drift here -- the bounds check matters more.)
-- ---------------------------------------------------------------------
