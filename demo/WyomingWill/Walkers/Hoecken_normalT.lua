...
-- Hoecken proportions (see header), uniformly rescaled so g_len still
-- matches the cube's top edge exactly as the Chebyshev version did:
--   S = g_len / native_ground = 2.5 / 2.3
-- native Hoecken values: crank=1.1, ground=2.3, coupler=2.8, rocker=2.8, tracer=5.65
...
local PARAM_INFO = {
  ...
  ["Left Speed"]  = { min = -8,   max = 8,   step = 0.1, -- WMS
                  comment = "front hip hinge motor target angular speed, linkage1/2 (live)" },
  ["Right Speed"] = { min = -8,   max = 8,   step = 0.1, -- WMS
...
-- ---------------------------------------------------------------------
-- The Hoecken B-point sits about 1.31 units higher over its cycle than
-- Chebyshev's did at the same mount height (checked numerically: B.y
-- ranges ~6.13..7.25 here vs. ~4.82..6.02 for the old Chebyshev linkage),
-- since Hoecken's rescaled coupler/rocker are longer. The baseline below
-- is shifted by that same ~1.3 to keep the same relationship to B's low
-- point that the original tuned "4.6" had -- a first-pass port, not a
-- from-scratch physics re-tune (Chebyshev's own "4.6" took real tuning
-- time to land on; this may still want a pass of the same kind).
-- WMS Was 4.6 (Chebyshev); ~5.9 for the Hoecken linkage's taller B path
-- WMS 5.3 is good for amp 0.8
