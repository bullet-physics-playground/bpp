...
-- back for the gear train, the escapement itself is run
-- counter-clockwise and the pallet is mirrored to still engage
-- correctly. Mirroring done properly on the actual STL geometry
-- (vertices negated in x, triangle winding flipped to keep normals
-- outward-facing), not just a coordinate hack.
...
-- ---------------------------------------------------------------------
-- Gear train + hands. Second and hour hands only -- no minute hand,
-- though gearC/gearD (stage 2) are still built since gearE/gearF
-- (stage 3, hour hand) are welded/meshed downstream of them and need
-- the full 5:1*12:1*12:1=720:1 reduction to reach a correct hour rate.
-- gearD just has nothing attached to it.
-- ---------------------------------------------------------------------
...
-- 144.44, same value since both meshes use an 8T pinion + 96T wheel at
-- the same module) from g2 itself -- a two-circle intersection. This
-- lets gearE (welded to gearD) mesh directly with a hollow hour wheel
-- centered exactly on g2's own axis, instead of the hour wheel sitting
-- off to the side.
-- ADJUSTABLE: rotates gearD clockwise around gearC (negative = clockwise),
-- preserving the gearC-gearD mesh distance exactly (rotating around
-- gearC guarantees this) while opening up the gearE-hourWheel gap the
-- idler needs. Valid range is roughly -1 to -20 (beyond -20.7, the gap
-- exceeds what any idler can reach at all). -15 gives a comfortable,
-- non-degenerate margin; smaller magnitudes (e.g. -10) give an even more
-- generous margin at the cost of a smaller gap.
...
-- 96T hour wheel that shares g2's own rotation axis (coaxial motion
-- works, like a real clock's hour wheel riding as a tube around the
-- minute/center arbor) -- 720:1 cumulative from the escape wheel.
...
-- (matching the second hand). Positioned via two-circle intersection,
-- same technique as gearD's own placement.
local sdlIdler, rpIdler = involute_gear_sdl(8, GEAR_MODULE, GEAR_PRESSURE, GEAR_TH)
-- Idler position solved via two-circle intersection: exactly
-- (rpE + rpIdler) from gearE, and exactly (rpIdler + rpHour) from g2
-- (=hourWheel's position). Automatically adapts if GEARD_ROTATION_DEG
-- above changes, since gearE's position depends on it.
local rpHour = GEAR_MODULE * 96 / 2 -- computed early (before the actual hourWheel SDL generation below) since needed here first
...
-- carries it -- must happen before its hinge is created, and rounded
-- to the nearest tooth pitch (360/96=3.75deg) so it stays in phase
-- with gearE. Reference angle unverified visually -- check on render.
do
  local t = os.date("*t")
...
-- COAXIAL HINGE: shares g2's own (x,y) pivot exactly (same pivot0.x/y
-- as g2's hinge below, c.pos subtracted), just at hourWheel's own z --
-- this is what makes it genuinely coaxial rather than merely
-- "positioned at the same spot", since the hour wheel rotates
-- independently of g2 at its own (much slower) rate while sharing the
-- same rotation axis in space.
